...
$loop = new WP_Query( array( 'post_type' => 'solutions', 'posts_per_page' => -1, 'order' => 'ASC' ) );
			...
			<?php if( have_rows('grid_content_box') ):
				while( have_rows('grid_content_box') ): the_row(); ?>
					...
					<?php if( have_rows('external_link_button') ): 
						...
						<?php else : ?>
						<a data-icon="C" class="et_pb_button hide-on-desktop" style="font-size: 16px;" href="#<?php echo sanitize_title_with_dashes( $title, $unused, $context = 'display' ); ?>">Learn More</a>
	...
	<?php while ( $loop->have_posts() ) : $loop->the_post(); ?>
		...
		<div id="<?php echo sanitize_title_with_dashes( $title, $unused, $context = 'display' ); ?>" <?php post_class( 'blurb' ); ?>>
			<h2><?php the_title(); ?></h2>
			<?php the_content(); ?>
